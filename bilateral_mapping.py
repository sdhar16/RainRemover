...
img_dirs = "./dc_bilateral"
model_dirs = "./models_bilateral"
...
dataset_training = RainyDataset('rainy-image-dataset/training', transform=img_transform,bilateralFilter=True)
...
for epoch in range(num_epochs):
    ...
    for index,data in enumerate(dataloader_training):
        ...
        bilateral = Variable(bilateral).cuda()

        
        # ===================forward=====================
        output = model( rainy_img - bilateral )
...
dataset_testing = RainyDataset('rainy-image-dataset/testing', transform=img_transform,bilateralFilter=True)
...
model.load_state_dict(torch.load("%s/conv_autoencoder_12.pth"%model_dirs))
